...
impl AiService for OpenaiClient {
    async fn generate_config(
        ...
        let content_str = resp_json["choices"][0]["message"]["content"]
            .as_str()
            .ok_or_else(|| "Failed to extract content from OpenAI response")?;

        let parsed: serde_json::Value = serde_json::from_str(content_str)?;
        let configs_value = parsed["configs"]
            .as_array()
            .ok_or_else(|| "Generated JSON lacks a 'configs' array")?;
    ...
    async fn repair_config(
        ...
        let content_str = resp_json["choices"][0]["message"]["content"]
            .as_str()
            .ok_or_else(|| "Failed to extract content from OpenAI repair response")?;

        let parsed: serde_json::Value = serde_json::from_str(content_str)?;
        let configs_value = parsed["configs"]
            .as_array()
            .ok_or_else(|| "Repaired JSON lacks a 'configs' array")?;
